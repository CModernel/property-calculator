    ...
    <div>
      ...
      {showAddChange && (
        <div className="mt-2 p-2 bg-gray-50 rounded border border-gray-200 space-y-2">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Starting month: {newStartMonth}</label>
            <input
              type="range"
              min="1"
              max="360"
        ...
        <div className="mt-2 space-y-1">
          {[...changes]
            ...
            .map((c) => (
              <div
                key={c.id}
                className="flex justify-between items-center px-2 py-1 bg-gray-50 border border-gray-200 rounded text-xs"
              >
                <span>
